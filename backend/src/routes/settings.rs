...
fn get_setting_bounds(key: &str) -> Option<SettingDef> {
    match key {
        ...
        "jwt_expiry_days" => Some(SettingDef { min: Some(1), max: Some(90) }),
        "m2m_token_expiry_hours" => Some(SettingDef { min: Some(1), max: Some(24) }),
        
        // Limits (per-org configurable)
        "max_servers_per_org" => Some(SettingDef { min: Some(1), max: Some(1000) }),
        "max_gateways_per_org" => Some(SettingDef { min: Some(1), max: Some(500) }),
        "max_pats_per_user" => Some(SettingDef { min: Some(1), max: Some(100) }),
        "max_service_accounts_per_org" => Some(SettingDef { min: Some(1), max: Some(100) }),
...
fn validate_setting_value(key: &str, value: &str) -> Result<(), (StatusCode, String)> {
    ...
    if let Some(def) = bounds {
        let parsed: i64 = value.parse()
            .map_err(|_| (StatusCode::BAD_REQUEST, format!("Value for '{}' must be an integer", key)))?;
        
        if let Some(min) = def.min {
            if parsed < min {
                return Err((StatusCode::BAD_REQUEST, format!("Value for '{}' must be at least {}", key, min)));
            }
        }
        
        if let Some(max) = def.max {
            if parsed > max {
                return Err((StatusCode::BAD_REQUEST, format!("Value for '{}' must be at most {}", key, max)));
            }
        }
    }
...
pub async fn list_settings(
    ...
    let can_edit = role.map(|(r,)| r == "owner" || r == "admin").unwrap_or(false);
    
    let response = SettingsWithMetadata {
        settings: settings.into_iter().map(|s| SettingResponse {
            key: s.setting_key,
...
pub async fn update_setting(
    ...
    let is_allowed = role.map(|(r,)| r == "owner" || r == "admin").unwrap_or(false);
    
    if !is_allowed {
        return Err((StatusCode::FORBIDDEN, "Only owner or admin can modify settings".to_string()));
    }
    ...
    if existing.is_none() {
        return Err((StatusCode::NOT_FOUND, format!("Setting '{}' not found", key)));
    }
    ...
    tracing::info!("Setting '{}' updated to '{}' by user {} in org {}", 
        key, payload.value, auth_user.user_id, auth_user.org_id);
...
pub async fn get_limits(
    ...
    let pats_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM personal_access_tokens WHERE user_id = $1 AND org_id = $2")
        .bind(user_id)
        .bind(org_id)
        .fetch_one(&state.db.pool)
        .await
    ...
    let sas_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM service_accounts WHERE org_id = $1")
        .bind(org_id)
        .fetch_one(&state.db.pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("{}: {}", error::DATABASE_ERROR, e)))?;
    ...
    let max_servers = get_org_setting_value(&state.db, org_id, "max_servers_per_org", "100").await.parse::<i64>().unwrap_or(100);
    let max_gateways = get_org_setting_value(&state.db, org_id, "max_gateways_per_org", "50").await.parse::<i64>().unwrap_or(50);
    let max_pats = get_org_setting_value(&state.db, org_id, "max_pats_per_user", "10").await.parse::<i64>().unwrap_or(10);
    let max_sas = get_org_setting_value(&state.db, org_id, "max_service_accounts_per_org", "20").await.parse::<i64>().unwrap_or(20);

    Ok(Json(LimitStatus {
        servers: LimitInfo { current: servers_count.0, max: max_servers, can_create: servers_count.0 < max_servers },
        gateways: LimitInfo { current: gateways_count.0, max: max_gateways, can_create: gateways_count.0 < max_gateways },
        pats: LimitInfo { current: pats_count.0, max: max_pats, can_create: pats_count.0 < max_pats },
        service_accounts: LimitInfo { current: sas_count.0, max: max_sas, can_create: sas_count.0 < max_sas },
    }))
}
