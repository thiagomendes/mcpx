...
use crate::models::server::{CreateServerRequest, Server, ServerResponse, UpdateServerRequest};
use crate::AppState;
...
const SQL_UPDATE_SERVER_ERROR: &str =
    "UPDATE servers SET status = 'unhealthy', health_error = $1, updated_at = NOW() WHERE id = $2";
...
pub async fn create_server(
    ...
    if current_count.0 >= max_servers {
        return Err((
            ...
            format!(
                "Server limit reached. Maximum {} servers per organization.",
                max_servers
            ),
        ));
    }
        ...
        .all(|c| c.is_alphanumeric() || c == '-' || c == '_')
    {
        return Err((
            ...
            error::INVALID_SERVER_NAME.to_string(),
        ));
    }
    ...
    let server = sqlx::query_as::<_, Server>(SQL_INSERT_SERVER)
        ...
        .bind(if payload.auth_type.starts_with("oauth") {
            Some(true)
        } else {
            None
        })
        ...
        if !api_key.is_empty() {
            ...
            sqlx::query(SQL_INSERT_CREDENTIAL)
                ...
                .map_err(|e| {
                    (
                        StatusCode::INTERNAL_SERVER_ERROR,
                        format!("{}: {}", ERR_FAILED_TO_STORE_CREDENTIAL, e),
                    )
                })?;
        }
    }
                    ...
                    match token_response {
                        Ok(resp) if resp.status().is_success() => {
                            if let Ok(json) = resp.json::<serde_json::Value>().await {
                                ...
                                {
                                    tracing::info!(
                                        "OAuth Client Credentials token obtained successfully"
                                    );
                                    (
                                        Some("Authorization".to_string()),
...
async fn test_mcp_connection(
    ...
    let client: RunningService<rmcp::RoleClient, _> = client_info
        .serve(transport)
        .await
