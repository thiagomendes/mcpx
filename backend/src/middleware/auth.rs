...
impl AuthUser {
    ...
    pub fn require(&self, permission: super::permissions::Permission) -> Result<(), (StatusCode, String)> {
    ...
    async fn from_request_parts(
        ...
        if token.starts_with("mcpx_pat_") {
            ...
            match pat_result {
                ...
                None => Err((StatusCode::UNAUTHORIZED, "Invalid or expired PAT".to_string())),
            }
            ...
            if let Ok(token_data) = decode::<M2MClaims>(
                ...
                if claims.token_type == "m2m" {
                    ...
                    let sa_id = uuid::Uuid::parse_str(&claims.sub)
