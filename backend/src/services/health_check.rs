...
pub async fn start_health_check_job(db: Database, encryption_key: String, interval_seconds: u64) {
    tracing::info!(
        "Starting health check job with {}s interval",
        interval_seconds
    );
...
async fn check_single_server(
    ...
    match result {
        ...
        Err(e) => {
            let (new_status, error_msg) =
                if e.contains("invalid_token") || e.contains("Token is not active") {
                    match try_refresh_token(db, server.id, encryption_key).await {
                        Ok(new_token) => {
                            ...
                            .await
                            {
                                ...
                                Err(e2) => ("unhealthy".to_string(), Some(e2)),
                            }
                        }
                        Err(_) => (
                            "pending_auth".to_string(),
                            Some("OAuth token expired and refresh failed".to_string()),
                        ),
                    }
                } else {
...
async fn discover_oauth_endpoints(server_url: &str) -> Result<(String, String), String> {
    ...
    let prm: ProtectedResourceMetadata = prm_response
        .json()
        .await
    ...
    let asm: AuthServerMetadata = asm_response
        .json()
        .await
...
async fn try_refresh_token(
    ...
    let (refresh_encrypted, org_id, dynamic_client_id) =
        token_row.ok_or("No refresh token available for this server")?;

    // 2. Fetch server OAuth config (token_url, client_id, and server URL for discovery)
    let server_row: Option<(Option<String>, Option<String>, String)> =
        sqlx::query_as("SELECT oauth_token_url, oauth_client_id, url FROM servers WHERE id = $1")
            .bind(server_id)
            .fetch_optional(&db.pool)
            .await
            .map_err(|e| format!("Failed to fetch server config: {}", e))?;

    let (configured_token_url, configured_client_id, server_url) =
        server_row.ok_or("Server not found")?;
    ...
    tracing::info!(
        "Attempting OAuth token refresh for server {} (client_id: {:?})",
        server_id,
        effective_client_id
    );
